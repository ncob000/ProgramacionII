...
{
	// Cambiar el color de la consola al color de relleno especificado.
	Color(colorRelleno);

	// Iterar a través de las filas y columnas del cuadrado para dibujar el relleno.
	for (int i = x1; i <= x2; i++)
	{
		for (int j = y1; j <= y2; j++)
		{
			// Posicionar el cursor en la consola y dibujar un bloque sólido (char(219)) para el relleno.
			gotoxy(i, j);
			printf("%c", char(219));
		}
...
{
	char Horizontal, Vertical, EsquinaSupDerecha, EsquinaSupIzquierda,
		EsquinaInfDerecha, EsquinaInfIzquierda, InterseccionLineaSuperior, InterseccionLineaIzquierda, InterseccionLineaDerecha, InterseccionCruz;
